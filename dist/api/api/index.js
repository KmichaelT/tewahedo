...
import { z } from "zod";
var adminWhitelist, insertAdminWhitelistSchema, users, insertUserSchema, upsertUserSchema, questions, insertQuestionSchema, answers, insertAnswerSchema, comments, insertCommentSchema, likes, insertLikeSchema, commentLikes, insertCommentLikeSchema, answerLikes, insertAnswerLikeSchema;
var init_schema = __esm({
  "shared/schema.ts"() {
    "use strict";
    adminWhitelist = pgTable("admin_whitelist", {
      email: text("email").primaryKey().notNull(),
      addedBy: text("added_by").notNull().default("system"),
      addedAt: timestamp("added_at").notNull().defaultNow()
    });
    insertAdminWhitelistSchema = createInsertSchema(adminWhitelist);
    users = pgTable("users", {
    ...
    upsertUserSchema = insertUserSchema.extend({
      id: z.string(),
    ...
    insertQuestionSchema = createInsertSchema(questions).pick({
      title: true,
      content: true,
      author: true,
      status: true,
    ...
    insertAnswerSchema = createInsertSchema(answers).pick({
      questionId: true,
      content: true,
      author: true,
      category: true,
    ...
    insertLikeSchema = createInsertSchema(likes).pick({
      questionId: true,
      userId: true
    });
    ...
    insertCommentLikeSchema = createInsertSchema(commentLikes).pick({
      commentId: true,
      userId: true
    });
    answerLikes = pgTable("answer_likes", {
      id: serial("id").primaryKey(),
      answerId: integer("answer_id").notNull(),
      userId: integer("user_id").notNull(),
      date: timestamp("date").notNull().defaultNow()
    });
    insertAnswerLikeSchema = createInsertSchema(answerLikes).pick({
      answerId: true,
      userId: true
    });
  }
...
var authMiddleware, requireAuth, requireAdmin;
var init_auth = __esm({
  "server/auth.ts"() {
    "use strict";
    init_storage();
...
var sampleQuestions, sampleAnswers;
  ...
  app2.use(async (req, res, next) => {
    try {
      ...
      if (authHeader && authHeader.startsWith("Bearer ")) {
        const idToken = authHeader.split("Bearer ")[1];
        try {
          const decodedToken = await admin3.auth().verifyIdToken(idToken);
...
var importRoutes = async () => {
  try {
    return await Promise.resolve().then(() => (init_routes(), routes_exports));
  } catch (error) {
    try {
      console.log("Trying alternative import path for routes");
      return await import(path2.join(__dirname, "../server/routes"));
    } catch (secondError) {
      ...
      try {
        ...
        return await import(path2.join(process.cwd(), "server/routes"));
      }
    }
  }
};
var importAuth = async () => {
  try {
    return await Promise.resolve().then(() => (init_firebaseAuth(), firebaseAuth_exports));
  } catch (error) {
    try {
      console.log("Trying alternative import path for auth");
      return await import(path2.join(__dirname, "../server/firebaseAuth"));
    } catch (secondError) {
      ...
      try {
        ...
        return await import(path2.join(process.cwd(), "server/firebaseAuth"));
      }
    }
  }
};
...
(async () => {
  try {
    const { setupAuth: setupAuth2 } = await importAuth();
    await setupAuth2(app);
    const { registerRoutes: registerRoutes2 } = await importRoutes();
...
app.get("*", (req, res) => {
  res.sendFile("index.html", { root: path2.join(process.cwd(), "dist/public") });
});
