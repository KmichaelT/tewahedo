...
const sampleAnswers: (Omit<InsertAnswer, "questionId"> & { questionTitle: string })[] = [
...
export async function seedSampleData() {
  ...
  try {
    ...
    for (const questionData of sampleQuestions) {
      ...
      // Add votes and comments for realism
      ...
      const newQuestion = await storage.createQuestion({
        ...
        votes,
        answers: 0, // Will be updated when answer is added
        comments,
      ...
      if (matchingAnswer) {
        ...
        await storage.updateQuestion(newQuestion.id, { 
          answers: 1,
          status: "published" // Ensure the question is published if it has an answer
        });
      }
    }
